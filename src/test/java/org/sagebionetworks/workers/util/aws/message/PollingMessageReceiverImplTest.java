...
public class PollingMessageReceiverImplTest {
	...
	public void testSemaphoreLockLessThanVisisibleTimeout(){
		config.setSemaphoreLockTimeoutSec(config.getMessageVisibilityTimeoutSec()-1);
		new PollingMessageReceiverImpl(mockAmazonSQSClient, config);
	}
	
	@Test
	public void testRunNullMessages() throws RecoverableMessageException{
		ReceiveMessageResult results = new ReceiveMessageResult();
		when(mockAmazonSQSClient.receiveMessage(any(ReceiveMessageRequest.class))).thenReturn(results);
	...
	@Test
	public void testRunEmptyMessages() throws RecoverableMessageException{
		ReceiveMessageResult results = new ReceiveMessageResult();
	...
	public void testRunTooMessages() throws RecoverableMessageException{
		ReceiveMessageResult results = new ReceiveMessageResult();
	...
	@Test 
	public void testOneMessage() throws RecoverableMessageException{
		PollingMessageReceiverImpl receiver = new PollingMessageReceiverImpl(mockAmazonSQSClient, config);
		
		//call under test
		receiver.run(mockProgressCallback);
		verify(mockProgressCallback, times(1)).progressMade(any(Message.class));
		verify(mockRunner, times(1)).run(any(ProgressCallback.class), any(Message.class));
		// The message should be deleted
		DeleteMessageRequest deleteMessageRequest = new DeleteMessageRequest();
		deleteMessageRequest.setQueueUrl(queueUrl);
		deleteMessageRequest.setReceiptHandle(message.getReceiptHandle());
		verify(mockAmazonSQSClient, times(1)).deleteMessage(deleteMessageRequest);
	}
	
	@Test 
	public void testMessageDeleteOnException() throws RecoverableMessageException{
		// setup the runner to throw an unknown exception
		doThrow(new IllegalArgumentException("Something was null")).when(mockRunner).run(any(ProgressCallback.class), any(Message.class));
	...
	@Test 
	public void testMessageNoDeleteRecoverableMessageException() throws RecoverableMessageException{
	...
	@Test
	public void testProgressCallbackThrottle() throws RecoverableMessageException{
		int timeout = 40;
		...
		doAnswer(new Answer<Void>() {
			...
			public Void answer(InvocationOnMock invocation) throws Throwable {
				ProgressCallback<Message> callback = (ProgressCallback<Message>) invocation.getArguments()[0];
		...
		receiver.run(mockProgressCallback);
		/*
		 * Progress should be made for the following:
		 * One time after the message received.
